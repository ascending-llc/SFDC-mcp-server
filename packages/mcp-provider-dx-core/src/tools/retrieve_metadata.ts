...
Retrieve X metadata from my org`,
  ...
  public async exec(
    ...
    if (input.directory && fs.existsSync(input.directory)) {
      ...
      console.error(
        `[retrieve_metadata] ⚠️  Directory not found (${input.directory}). Continuing with current working directory.`
      );
    }

    if (!extra) {
      console.error(`[retrieve_metadata] ❌ No OAuth context provided`);
      return textResponse(
